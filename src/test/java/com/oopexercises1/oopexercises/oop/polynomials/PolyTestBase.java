...
import com.oopexercises.oop.polynomials.Poly;
...
abstract class PolyTestBase {
    ...
    void coefficient() {
        ...
        //aggiunti test per controllare che il grado fornito sia ammissibile
        assertThrows(IllegalArgumentException.class, () -> p.coefficient(6));
        assertThrows(IllegalArgumentException.class, () -> p.coefficient(-1));
    }
    ...
    //aggiunto per testare il metodo toString
    @Test
    void testToString() {
        assertEquals("Poly{1.0 + 2.0 * x^1 + 3.0 * x^2 + 4.0 * x^3}", p.toString());
    }
}
